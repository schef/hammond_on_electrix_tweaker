...
void MidiPlayer::init()
{
    try
    {
        ...
        chooseMidiInPort(_midiInTweaker, (char *)VMPK_SEARCH_NAME, (char *)MIDI_IN_TWEAKER_PORT_NAME);
        ...
        chooseMidiOutPort(_midiOutTweaker, (char *)VMPK_SEARCH_NAME, (char *)MIDI_OUT_TWEAKER_PORT_NAME);

        _midiOut = new RtMidiOut(RtMidiIn::Api::UNSPECIFIED, MIDI_NAME);
        chooseMidiOutPort(_midiOut, (char *)INVALID_SEARCH_NAME, (char *)MIDI_OUT_PORT_NAME);
    }
...
void MidiPlayer::chooseMidiInPort(RtMidiIn *rtmidi, char *searchName, char *portName)
{
...
void MidiPlayer::chooseMidiOutPort(RtMidiOut *rtmidi, char *searchName, char *portName)
{
    ...
    rtmidi->openVirtualPort(portName);
    printf(FILE_NAME "midiOutPort not found, opening virtual port for testing.\n");
}

void MidiPlayer::registerCallbackFunction(void (*callbackFunction)(double deltatime, std::vector<unsigned char> *message, void *))
{
    printf(FILE_NAME "registerCallbackFunction\n");
    _callbackFunction = callbackFunction;
}

void MidiPlayer::callbackFunction(double deltatime, std::vector<unsigned char> *message, void *userData)
{
...
void MidiPlayer::staticCallbackFunction(double deltatime, std::vector<unsigned char> *message, void *userData)
{
